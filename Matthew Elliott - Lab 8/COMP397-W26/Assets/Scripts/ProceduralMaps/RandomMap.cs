...
public class RandomMap : MonoBehaviour
{
    ...
    private InputAction playerRebuild;
    ...
    private void Start()
    {
        ...
        playerRebuild = InputSystem.actions.FindAction("Player/RebuildMap");
        playerRebuild.started += Rebuild;
    }
    ...
    private void Rebuild(InputAction.CallbackContext context)
    {
        RebuildPerlinMap();
    }
    private void BuildPerlinNoiseMap()
    {
        for (int row = 0; row < depth; row++)
        {
            ...
            {
                if (row == 0 && col == 0) { continue; }
                float perlinNoiseValue = GetPerlinNoiseValue(col, row);
                Vector3 pos = new Vector3(col * 50, 0f, row * 50);
    ...
    private GameObject GenerateTileOnPerlinNoise(float noiseValue)
    {
        switch (noiseValue)
        {
            case < 0.2f: // Less than 20%
                return Instantiate(prefabTilesList[0]); // Water
            case < 0.4f: // Less than 40%
                return prefabTilesList[1]; // Grass
            case < 0.6f: // Less than 60%
                return prefabTilesList[2]; // Road
            case < 0.8f: // Less than 80%
                return prefabTilesList[3]; // Ground
            case <= 1f:
                return prefabTilesList[4]; // Lava
            default:
                return prefabTilesList[1]; // Default will be Grass
        }
    }
